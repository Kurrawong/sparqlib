...
class TestRobustness:
    ...
    def test_quad_data_robustness(self):
        ...
        # simple quads tree
        quads = Tree("quads", [
            Tree("triples_template", [
                Tree("triples_same_subject", [
                    Tree("var_or_term", [Tree("var", [Token("VAR", "?s")])]),
                    Tree("property_list_not_empty", [
                        Tree("verb_object_list", [
                            Tree("verb", [Tree("var_or_iri", [Tree("var", [Token("VAR", "?p")])])]),
                            Tree("object_list", [
                                Tree("object", [Tree("graph_node", [Tree("var_or_term", [Tree("var", [Token("VAR", "?o")])])])])
                            ])
                        ])
                    ])
                ])
        ...
        assert "?p" in result
        assert "?o" in result
        assert "garbage" not in result # Verified behavior: explicit selection ignores others
